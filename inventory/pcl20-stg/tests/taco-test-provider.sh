...
PROVIDER_NAME_TEMP=$(openstack network list | grep provider-net | awk '{print $4}')
if [ "x${PROVIDER_NAME_TEMP}" != "xprovider-net" ]; then
...
--allocation-pool start=172.27.126.193,end=172.27.126.225 --dns-nameserver 8.8.8.8 provider-subnet
fi
echo "Done"
...
--file /home/taco/cirros-0.4.0-x86_64-disk.img \
cirros-0.4.0
