...
만일, 이미 설치된 클러스터에 패치 작업이나 재설치 등을 하고 싶을 경우는, allow_redeployment 변수를 'true'로 설정하고 진행하면 된다.
